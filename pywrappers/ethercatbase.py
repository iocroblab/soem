...
uint8 = c_ubyte
uint16 = c_ushort
ec_setupdatagram = _libraries['libsoem.so'].ec_setupdatagram
ec_setupdatagram.restype = c_int
ec_setupdatagram.argtypes = [c_void_p, uint8, uint8, uint16, uint16, uint16, c_void_p]
boolean = c_ubyte
ec_adddatagram = _libraries['libsoem.so'].ec_adddatagram
ec_adddatagram.restype = c_int
ec_adddatagram.argtypes = [c_void_p, uint8, uint8, boolean, uint16, uint16, uint16, c_void_p]
ec_BWR = _libraries['libsoem.so'].ec_BWR
ec_BWR.restype = c_int
ec_BWR.argtypes = [uint16, uint16, uint16, c_void_p, c_int]
ec_BRD = _libraries['libsoem.so'].ec_BRD
ec_BRD.restype = c_int
ec_BRD.argtypes = [uint16, uint16, uint16, c_void_p, c_int]
ec_APRD = _libraries['libsoem.so'].ec_APRD
ec_APRD.restype = c_int
ec_APRD.argtypes = [uint16, uint16, uint16, c_void_p, c_int]
ec_ARMW = _libraries['libsoem.so'].ec_ARMW
ec_ARMW.restype = c_int
ec_ARMW.argtypes = [uint16, uint16, uint16, c_void_p, c_int]
ec_FRMW = _libraries['libsoem.so'].ec_FRMW
ec_FRMW.restype = c_int
ec_FRMW.argtypes = [uint16, uint16, uint16, c_void_p, c_int]
ec_APRDw = _libraries['libsoem.so'].ec_APRDw
ec_APRDw.restype = uint16
ec_APRDw.argtypes = [uint16, uint16, c_int]
ec_FPRD = _libraries['libsoem.so'].ec_FPRD
ec_FPRD.restype = c_int
ec_FPRD.argtypes = [uint16, uint16, uint16, c_void_p, c_int]
int16 = c_short
ec_FPRDw = _libraries['libsoem.so'].ec_FPRDw
ec_FPRDw.restype = uint16
ec_FPRDw.argtypes = [uint16, uint16, int16]
ec_APWRw = _libraries['libsoem.so'].ec_APWRw
ec_APWRw.restype = c_int
ec_APWRw.argtypes = [uint16, uint16, uint16, c_int]
ec_APWR = _libraries['libsoem.so'].ec_APWR
ec_APWR.restype = c_int
ec_APWR.argtypes = [uint16, uint16, uint16, c_void_p, c_int]
ec_FPWRw = _libraries['libsoem.so'].ec_FPWRw
ec_FPWRw.restype = c_int
ec_FPWRw.argtypes = [uint16, uint16, uint16, c_int]
ec_FPWR = _libraries['libsoem.so'].ec_FPWR
ec_FPWR.restype = c_int
ec_FPWR.argtypes = [uint16, uint16, uint16, c_void_p, c_int]
uint32 = c_uint
ec_LRW = _libraries['libsoem.so'].ec_LRW
ec_LRW.restype = c_int
ec_LRW.argtypes = [uint32, uint16, c_void_p, c_int]
ec_LRD = _libraries['libsoem.so'].ec_LRD
ec_LRD.restype = c_int
ec_LRD.argtypes = [uint32, uint16, c_void_p, c_int]
ec_LWR = _libraries['libsoem.so'].ec_LWR
ec_LWR.restype = c_int
ec_LWR.argtypes = [uint32, uint16, c_void_p, c_int]
int64 = c_longlong
ec_LRWDC = _libraries['libsoem.so'].ec_LRWDC
ec_LRWDC.restype = c_int
ec_LRWDC.argtypes = [uint32, uint16, c_void_p, uint16, POINTER(int64), c_int]
int8 = c_byte
int32 = c_int
uint64 = c_ulonglong
ec_bufT = uint8 * 1518
...
__all__ = ['__uint16_t', 'ECT_REG_DLSTAT',
           ...
           'ECT_MBXT_AOE', 'EC_BUF_COMPLETE', 'ec_adddatagram',
           'timeval', '__uint32_t', 'ECT_SDO_UP_REQ', 'ec_cmdtype',
           '__time_t', 'ECT_UNSIGNED32', 'itimerval', 'ECT_SII_SM',
           'ECT_REG_TYPE', 'uint16', 'ECT_REG_PDICTL', 'ec_bufstate',
           'ECT_MBXT_COE', 'EC_STATE_PRE_OP', '__qaddr_t',
           'ECT_SDO_ABORT', '__u_short', '__loff_t',
           'ECT_SII_MBXPROTO', 'ECT_SDO_DOWN_INIT_CA',
           'ECT_SOE_EMERGENCY', 'ECT_GET_ODLIST_REQ', 'ec_FPWRw',
           '__int32_t', 'ECT_COES_SDOREQ', 'ECT_REG_ESCSUP',
           '__itimer_which_t', 'ECT_DOMAIN', 'ECT_REG_DCTIMEFILT',
           'ECT_REG_SM1', 'ECT_SII_MANUF', 'ECT_REG_SM3',
           'ECT_GET_OE_REQ', 'ECT_SOE_NOTIFICATION', 'ec_APWR',
           'ECT_REG_IRQMASK', 'sigset_t', 'ECT_MBXT_EOE',
           'ECT_REG_FMMU0', 'ECT_REG_FMMU1', 'ECT_REG_FMMU2',
           'ECT_REG_FMMU3', 'ECT_GET_OD_RES', 'ec_datatype',
           'ECT_GET_OD_REQ', 'boolean', 'ec_LWR', 'ECT_REG_EEPADR',
           '__ssize_t', 'ECT_REG_SM1ACT', '__nlink_t', 'ECT_REAL32',
           'ECT_REG_DCSOF', 'ECT_SII_REV', 'futimes', '__uint64_t',
           'fd_set', 'ECT_BOOLEAN', 'ECT_REG_DCSYSDIFF', '__off64_t',
           'ECT_REG_STADR', '__fd_mask', 'ec_LRWDC', 'int16',
           'EC_ECMD_NOP', 'ec_FPRDw', 'EC_ERR_TYPE_SOE_ERROR',
           'ECT_REG_DLCTL', '__id_t', 'getitimer', '__sigset_t',
           '__clockid_t', '__useconds_t', 'ECT_UNSIGNED16', 'adjtime',
           'ECT_COES_TXPDO', '__timer_t', 'ECT_REG_DCCUC',
           'EC_ERR_TYPE_FOE_ERROR', 'ECT_SII_TXMBXADR', 'select',
           'ECT_INTEGER8', 'ec_FPRD', 'uint32', 'ECT_REG_DCSYSTIME',
           'EC_ERR_ALREADY_INITIALIZED', 'EC_CMD_BRW', 'ec_err_type',
           'ECT_SII_BOOTRXMBX', 'ECT_INTEGER64', 'ECT_VISIBLE_STRING',
           'EC_CMD_LRD', 'ECT_FOE_BUSY', 'ec_APRD', 'EC_CMD_BRD',
           'EC_STATE_ACK', '__blkcnt64_t', 'ECT_SII_RXMBXADR',
           '__u_long', 'ec_APWRw', 'ECT_GET_OE_RES', 'EC_CMD_LRW',
           'EC_ERR_TYPE_PACKET_ERROR', 'ECT_COES_SDOINFO',
           'ECT_FOE_READ', 'ec_APRDw', '__fsfilcnt64_t',
           'EC_CMD_APRD', '__blkcnt_t', 'ECT_BIT1', 'ECT_BIT2',
           'ECT_BIT3', 'ECT_BIT4', 'ECT_BIT5', 'ECT_BIT6', 'ECT_BIT7',
           'ECT_BIT8', '__ino_t', 'ECT_REG_SM2', '__rlim64_t',
           'EC_ERR_OK', 'EC_CMD_APRW', 'timespec', 'EC_CMD_NOP',
           'ECT_SII_ID', 'setitimer', 'EC_BUF_RCVD', 'ECT_FOE_WRITE',
           ...
           'ITIMER_PROF', 'EC_CMD_APWR', 'ec_ARMW', 'ECT_REG_ALCTL',
           'EC_STATE_SAFE_OP', 'ECT_SII_PDO', 'ec_LRW',
           ...
           'ECT_REG_DLALIAS', 'ec_BRD', 'EC_ERR_TYPE_FOE_BUF2SMALL',
           'EC_STATE_INIT', 'EC_ERR_TIMEOUT', 'ECT_REG_DLPORT',
           'ECT_MBXT_ERR', 'EC_BUF_TX', 'ec_bufT',
           'ECT_SDO_UP_REQ_CA', 'ec_FRMW', 'ECT_FOE_ACK',
           '__socklen_t', 'ECT_REG_DCSYSDELAY', 'fd_mask',
           'ECT_FOE_DATA', 'EC_CMD_FPRW', 'ec_LRD', 'ECT_INTEGER32',
           '__ino64_t', 'ECT_REG_EEPCFG', 'ec_etherheadert',
           'ECT_UNICODE_STRING', 'EC_CMD_BWR', 'EC_BUF_ALLOC',
           'ec_FPWR', '__caddr_t', 'ECT_REG_PORTDES',
           'ECT_TIME_OF_DAY', '__u_int', '__clock_t', 'timezone',
           '__fsblkcnt_t', 'ECT_GET_ODLIST_RES', 'ECT_UNSIGNED8',
           'ECT_REG_DCSYNCACT', 'ECT_REG_DCSYSOFFSET', 'time_t',
           'EC_CMD_FRMW', 'EC_STATE_BOOT', 'EC_CMD_ARMW',
           'EC_STATE_OPERATIONAL', 'ECT_MBXT_SOE',
           'ECT_COES_RXPDO_RR', '__gid_t', '__fsid_t',
           'ECT_UNSIGNED64', 'ECT_REG_DCCYCLE1', 'ECT_REG_DCCYCLE0',
           'ECT_REG_EEPSTAT', '__u_quad_t', 'ECT_COES_SDORES',
           'ECT_COES_RXPDO', 'lutimes', 'EC_ECMD_WRITE', 'ec_BWR',
           'EC_ECMD_READ', 'ec_comt', '__itimer_which', '__pid_t',
           'ECT_INTEGER24', 'ECT_REG_SM0STAT', 'ECT_SOE_WRITERES',
           '__fsblkcnt64_t', 'ECT_SOE_WRITEREQ', 'int32',
           'ECT_SII_MBXSIZE', 'ec_setupdatagram', 'ECT_REG_DCTIME1',
