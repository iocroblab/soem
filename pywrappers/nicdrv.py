...
ec_rxbuf = (ec_bufT * 16).in_dll(_libraries['libsoem.so'], 'ec_rxbuf')
ec_txbuf = (ec_bufT * 16).in_dll(_libraries['libsoem.so'], 'ec_txbuf')
ec_txbuf2 = (ec_bufT).in_dll(_libraries['libsoem.so'], 'ec_txbuf2')
ec_txbuflength = (c_int * 16).in_dll(_libraries['libsoem.so'], 'ec_txbuflength')
ec_txbuflength2 = (c_int).in_dll(_libraries['libsoem.so'], 'ec_txbuflength2')
ec_incnt = (c_int).in_dll(_libraries['libsoem.so'], 'ec_incnt')
ec_errcnt = (c_int).in_dll(_libraries['libsoem.so'], 'ec_errcnt')
ec_redstate = (c_int).in_dll(_libraries['libsoem.so'], 'ec_redstate')
hlp_txtime = (c_int).in_dll(_libraries['libsoem.so'], 'hlp_txtime')
hlp_rxtime = (c_int).in_dll(_libraries['libsoem.so'], 'hlp_rxtime')
sockhandle = (c_int).in_dll(_libraries['libsoem.so'], 'sockhandle')
sockhandle2 = (c_int).in_dll(_libraries['libsoem.so'], 'sockhandle2')
priMAC = (uint16 * 3).in_dll(_libraries['libsoem.so'], 'priMAC')
secMAC = (uint16 * 3).in_dll(_libraries['libsoem.so'], 'secMAC')
ec_setupnic = _libraries['libsoem.so'].ec_setupnic
ec_setupnic.restype = c_int
ec_setupnic.argtypes = [STRING, c_int]
ec_closenic = _libraries['libsoem.so'].ec_closenic
ec_closenic.restype = c_int
ec_closenic.argtypes = []
ec_setupheader = _libraries['libsoem.so'].ec_setupheader
ec_setupheader.restype = None
ec_setupheader.argtypes = [c_void_p]
ec_setbufstat = _libraries['libsoem.so'].ec_setbufstat
ec_setbufstat.restype = None
ec_setbufstat.argtypes = [uint8, c_int]
ec_getindex = _libraries['libsoem.so'].ec_getindex
ec_getindex.restype = uint8
ec_getindex.argtypes = []
ec_outframe = _libraries['libsoem.so'].ec_outframe
ec_outframe.restype = c_int
ec_outframe.argtypes = [uint8, c_int]
ec_outframe_red = _libraries['libsoem.so'].ec_outframe_red
ec_outframe_red.restype = c_int
ec_outframe_red.argtypes = [uint8]
ec_waitinframe = _libraries['libsoem.so'].ec_waitinframe
ec_waitinframe.restype = c_int
ec_waitinframe.argtypes = [uint8, c_int]
ec_srconfirm = _libraries['libsoem.so'].ec_srconfirm
ec_srconfirm.restype = c_int
ec_srconfirm.argtypes = [uint8, c_int]
__all__ = ['__uint16_t', 'ECT_REG_DLSTAT',
           ...
           'ECT_SDO_UP_REQ', 'ec_waitinframe', 'ec_cmdtype',
           '__time_t', 'ECT_UNSIGNED32', 'itimerval', 'ECT_SII_MANUF',
           'ECT_REG_TYPE', 'uint16', 'ECT_REG_PDICTL', 'ECT_MBXT_COE',
           'EC_STATE_PRE_OP', '__qaddr_t', 'ECT_SDO_ABORT',
           '__u_short', '__loff_t', 'ECT_SII_MBXPROTO',
           'ECT_GET_ODLIST_RES', 'ECT_SOE_EMERGENCY',
           'ECT_GET_ODLIST_REQ', '__int32_t', 'ECT_COES_SDOREQ',
           'ECT_REG_ESCSUP', '__itimer_which_t', 'ECT_DOMAIN',
           'ECT_REG_DCTIMEFILT', 'ECT_REG_SM1', 'ECT_SII_SM',
           'ECT_REG_SM3', 'ECT_GET_OE_REQ', 'ECT_SOE_NOTIFICATION',
           ...
           'ECT_REG_DCSOF', 'ECT_SII_REV', 'futimes', 'ec_incnt',
           '__uint64_t', 'hlp_rxtime', 'fd_set', 'ECT_BOOLEAN',
           'ECT_REG_DCSYSDIFF', '__off64_t', 'ECT_UNSIGNED8',
           '__fd_mask', 'int16', 'EC_ECMD_NOP', 'ec_bufT',
           'EC_ERR_TYPE_SOE_ERROR', 'ECT_REG_DLCTL', 'ec_rxbuf',
           '__id_t', 'getitimer', '__sigset_t', '__clockid_t',
           '__useconds_t', 'ECT_UNSIGNED16', 'ec_closenic', 'adjtime',
           'ECT_COES_TXPDO', '__timer_t', 'ECT_REG_DCCUC',
           'EC_ERR_TYPE_FOE_ERROR', 'sockhandle', 'ECT_SII_TXMBXADR',
           'select', 'ECT_INTEGER8', 'ec_bufstate', 'uint32',
           ...
           'ECT_FOE_BUSY', 'EC_CMD_BRD', 'ECT_REG_EEPSTAT',
           '__blkcnt64_t', 'ECT_SII_RXMBXADR', '__u_long',
           'ECT_GET_OE_RES', 'EC_CMD_LRW', 'ec_outframe_red',
           'EC_ERR_TYPE_PACKET_ERROR', 'EC_STATE_ACK',
           'EC_STATE_INIT', '__fsfilcnt64_t', 'EC_CMD_APRD',
           '__blkcnt_t', 'ec_errcnt', 'ECT_BIT1', 'ECT_UNSIGNED64',
           ...
           'sockhandle2', 'ECT_SII_ID', 'setitimer', 'EC_BUF_RCVD',
           'ECT_FOE_WRITE', 'EC_ERR_NOT_INITIALIZED', '__mode_t',
           'futimesat', 'ECT_SII_STRING', 'ECT_INTEGER16',
           'ECT_REG_EEPDAT', '__blksize_t', 'ECT_SII_BOOTTXMBX',
           'ECT_COES_EMERGENCY', 'N9ec_errort4DOT_22E', '__off_t',
           'secMAC', '__timezone_ptr_t', 'ECT_REG_ALIAS',
           '__intptr_t', 'EC_ECMD_RELOAD', 'ECT_REG_SM1CONTR',
           'EC_ERR_TYPE_EMERGENCY', 'ec_state', 'ECT_REG_SM0',
           'ECT_REG_EEPCTL', 'ECT_SDO_SEG_UP_REQ', 'EC_ERR_NOK',
           '__suseconds_t', 'ECT_SII_GENERAL', 'ECT_FOE_ERROR',
           'priMAC', 'EC_CMD_APWR', 'ECT_REG_ALCTL',
           'EC_STATE_SAFE_OP', 'ec_txbuflength2', 'ECT_SII_PDO',
           'ECT_SOE_READREQ', 'ECT_SOE_READRES', 'EC_BUF_EMPTY',
           '__uint8_t', 'ECT_REAL64', 'EC_BUF_ALLOC', '__u_char',
           '__rlim_t', 'uint8', 'ec_txbuf', 'ec_setupnic',
           '__sig_atomic_t', 'gettimeofday', 'EC_CMD_FPRD',
           'ECT_REG_ALSTAT', 'ec_setupheader', 'ECT_TIME_DIFFERENCE',
           'ec_txbuflength', 'pselect', 'int8', 'hlp_txtime',
           'ECT_SDOINFO_ERROR', 'uint64', 'ECT_REG_DLALIAS',
           'EC_ERR_TYPE_FOE_BUF2SMALL', 'ECT_FOE_READ',
           'EC_ERR_TIMEOUT', 'ECT_REG_DLPORT', 'ec_txbuf2',
           'ECT_MBXT_ERR', 'EC_BUF_TX', 'ECT_SDO_UP_REQ_CA',
           'ECT_FOE_ACK', '__socklen_t', 'ec_getindex', 'ec_outframe',
           'ECT_REG_DCSYSDELAY', 'fd_mask', 'ECT_FOE_DATA',
           'EC_CMD_FPRW', 'ec_redstate', 'ECT_INTEGER32', '__ino64_t',
           'ECT_REG_EEPCFG', 'ec_etherheadert', 'ECT_UNICODE_STRING',
           'EC_CMD_BWR', 'ITIMER_REAL', '__caddr_t',
           ...
           'ec_setbufstat', '__u_quad_t', 'ECT_COES_SDORES',
           'ECT_COES_RXPDO', 'lutimes', 'EC_ECMD_WRITE',
           ...
           'ECT_SII_MBXSIZE', 'ECT_REG_DCTIME1', 'ECT_REG_DCTIME0',
           ...
           'ECT_UNSIGNED24', 'ec_srconfirm', 'ec_errort', '__quad_t',
           '__int64_t', 'int64', 'ECT_COES_SDOINFO', 'ec_ecmdtype',
